...
(function() {
    ...
    const tiers = {
        7: "S+", 13: "S", 17: "S-", 45: "A+", 70: "A", 94: "B+", 131: "B",
        173: "C", 271: "D", 306: "F"
    };
    ...
    // This is the original hardcoded list.
    ...
    function loadOwned() {
        ...
        if (storedData) {
            ...
            // If nothing is in storage, use the default set and save it.
            owned = defaultOwnedSet;
            saveOwned();
        }
    }
    ...
    if (window.location.href.startsWith("https://battlecats.miraheze.org/wiki/Category")) {
        document.querySelectorAll(".gacha-banner").forEach(banner => {
            ...
            const originalBannerText = collapsible.innerText;
            const allUnitStages = banner.querySelectorAll(".collapsible-units");

            const allUnitImagesInBanner = banner.querySelectorAll(".collapsible-units img");

            const updateBannerCount = () => {
                const ownedCount = Array.from(allUnitImagesInBanner).filter(img => {
                    const catNum = parseInt(img.dataset.catNum, 10);
                    return owned.has(catNum);
                }).length;
                collapsible.innerText = `${ownedCount}/${allUnitImagesInBanner.length}\t${originalBannerText}`;
            };

            allUnitStages.forEach(stage => {
                stage.querySelectorAll("img").forEach(unit => {
                    ...
                    const updateUnitVisuals = () => {
                        ...
                        const baseAlt = unit.alt.includes("(") ? unit.alt.slice(0, unit.alt.indexOf("(")).trim() : unit.alt;
                        const rankInfo = catRank ? `${tiers[Object.keys(tiers).find(x => catRank <= x)]} ${catRank}` : '';
                        unit.alt = `${baseAlt} (${isOwned ? "O" : "U"} ${rankInfo})`.trim();

                        // Set parent title for tooltip
                    ...
                    updateUnitVisuals(); // Set initial state
                });
            });
        ...
        fetch('https://battlecats.miraheze.org/wiki/Module:Cats/images.csv?action=raw')
            ...
            .then(data => {
                ...
                document.querySelectorAll(".nyanko img").forEach(unit => {
                    ...
                    } else if (unit.src.endsWith("_m00.png")) { // Heuristic for eggs
        ...
        if (headingSpan && catNumElem && catNumElem.lastChild.previousSibling) {
            ...
            const updateButtonState = () => {
                ...
                toggleButton.style.backgroundColor = isOwned ? '#28a745' : '#dc3545'; // green : red
